<template>
	<div class="p-6">
		...
		<div class="space-y-2 mb-4">
			...
			<label class="inline-flex gap-1 min-w-[75%]">
				...
				<input
					...
					step="3"
					v-model="topN"
					class="border rounded bg-gray-500 w-[75%]"
				/>
			...
			<button
				@click="analyze"
				class="px-3 py-1 rounded mt-2 bg-indigo-600 text-white"
			>
				Analyze IT!
			</button>
		</div>
		...
		<div class="mt-4 p-2 border rounded bg-gray-500 relative overflow-hidden">
			<div
				v-if="loading"
				class="bg-gray-500 absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center"
			>
	...
	try {
		...
		for (const cssURL of cssFiles) {
			try {
				...
				for (const [prop, cnt] of Object.entries(counts))
					totalCounts[prop] = (totalCounts[prop] || 0) + cnt
			} catch (e) {
	...
	try {
		...
		if (repo.value) {
			repos = await fetchJSON(
				`${apiURL}/repos?username=${username.value}&repo=${repo.value}`
			)
		} else {
